...
public abstract class AbstractJobDao implements JobDao {

    protected abstract Logger getLogger();
    protected abstract DataSource getDataSource();
    protected abstract String getShardHashExpression();
    protected abstract void setUuidParam(PreparedStatement ps, int idx, UUID value) throws SQLException;
    protected abstract UUID getUuidFromResultSet(ResultSet rs, String column) throws SQLException;
    protected abstract String getJsonCast();
    ...
    public void insert(ScheduledJob job) {
        ...
        try (Connection conn = getDataSource().getConnection();
            ...
            getLogger().errorf(e, "Failed to insert job %s", job.getId());
    ...
    public boolean insertIfNotExistsByKey(ScheduledJob job) {
        ...
        try (Connection conn = getDataSource().getConnection();
            ...
            getLogger().errorf(e, "Failed to insertIfNotExistsByKey job %s", job.getId());
            throw new DaoException("Failed to insert job", e);
        }
    }
    ...
    public boolean update(ScheduledJob job) {
        String sql = """
            UPDATE scheduled_jobs SET
                ...
                cron_expression = ?, cron_until = ?, cron_repeat = ?, cron_run_count = ?,
                run_at = ?, effective_run_at = ?, arrived_at = ?,
                destination_topic = ?, message_key = ?, message_value = ?, headers = ?%s, advisory_headers_pattern = ?,
                state = ?, max_retries = ?, retry_count = ?, version = ?,
                predecessor_id = ?, sequence_num = ?,
        ...
        try (Connection conn = getDataSource().getConnection();
            ...
            setUuidParam(ps, idx++, job.getPredecessorId());
            ...
            setUuidParam(ps, idx++, job.getId());
            ps.setInt(idx++, job.getVersion() - 1);

            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            getLogger().errorf(e, "Failed to update job %s", job.getId());
    ...
    public Optional<ScheduledJob> findById(UUID id) {
        ...
        try (Connection conn = getDataSource().getConnection();
            ...
            getLogger().errorf(e, "Failed to find job %s", id);
    ...
    public List<ScheduledJob> findWaitingByPredecessor(UUID predecessorId) {
        ...
        return queryJobs(sql, ps -> setUuidParam(ps, 1, predecessorId));
    }
    ...
    public List<ScheduledJob> findPendingForShard(int shardIndex, int shardCount) {
        String sql = """
            ...
            """.formatted(getShardHashExpression());
    ...
    public List<ScheduledJob> findPendingForShardExcluding(int shardIndex, int shardCount, Set<UUID> excludeIds) {
        ...
        String sql = """
            ...
            WHERE state = 'PENDING'
              ...
              AND id NOT IN (%s)
            ORDER BY effective_run_at
            """.formatted(getShardHashExpression(), placeholders);
    ...
    public long countJobs(List<JobState> states, String jobKey, String destination) {
        ...
        try (Connection conn = getDataSource().getConnection();
            ...
            getLogger().errorf(e, "Failed to count jobs");
    ...
    public boolean acquire(UUID jobId, String acquiredBy, int expectedVersion) {
        ...
        try (Connection conn = getDataSource().getConnection();
            ...
            setUuidParam(ps, 3, jobId);
            ps.setInt(4, expectedVersion);
            return ps.executeUpdate() > 0;
        } catch (SQLException e) {
            getLogger().errorf(e, "Failed to acquire job %s", jobId);
    ...
    public JobStats getStats() {
        ...
        try (Connection conn = getDataSource().getConnection();
            ...
            getLogger().errorf(e, "Failed to get job stats");
    ...
    protected void setJobParameters(PreparedStatement ps, ScheduledJob job) throws SQLException {
        int idx = 1;
        setUuidParam(ps, idx++, job.getId());
        ps.setString(idx++, job.getJobKey());
        ps.setString(idx++, job.getKeyPolicy().name());
    ...
    protected ScheduledJob mapJob(ResultSet rs) throws SQLException {
        ScheduledJob job = new ScheduledJob();
        job.setId(getUuidFromResultSet(rs, "id"));
        ...
        job.setPredecessorId(getUuidFromResultSet(rs, "predecessor_id"));
    ...
    protected List<ScheduledJob> queryJobs(String sql, SqlConsumer<PreparedStatement> paramSetter) {
        ...
        try (Connection conn = getDataSource().getConnection();
            ...
            getLogger().errorf(e, "Failed to query jobs");
    ...
    protected Instant toInstant(Timestamp ts) {
        return ts != null ? ts.toInstant() : null;
    }

    protected void setNullableInt(PreparedStatement ps, int idx, Integer value) throws SQLException {
        if (value != null) {
            ps.setInt(idx, value);
        } else {
    ...
    protected Integer getNullableInt(ResultSet rs, String column) throws SQLException {
