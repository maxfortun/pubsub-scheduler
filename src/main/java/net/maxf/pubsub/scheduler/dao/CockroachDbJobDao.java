...
@ApplicationScoped
@CockroachDb
public class CockroachDbJobDao implements JobDao {

    private static final Logger LOG = Logger.getLogger(CockroachDbJobDao.class);
    ...
    public List<ScheduledJob> findPendingForShard(int shardIndex, int shardCount) {
        String sql = """
            SELECT * FROM scheduled_jobs
            WHERE state = 'PENDING'
              AND mod(abs(fnv32(COALESCE(job_key, id::text)::bytes)), ?) = ?
    ...
    public JobStats getStats() {
        String sql = "SELECT state, COUNT(*) as cnt FROM scheduled_jobs GROUP BY state";
