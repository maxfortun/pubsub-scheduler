#!/bin/bash
# Run containerized integration tests against all scheduler flavors in parallel
...
docker rm -f integration-test-postgres integration-test-mysql integration-test-cockroach integration-test-activemq 2>/dev/null || true

echo ""
echo "Starting integration tests (all 4 flavors in parallel)..."

# Start all 4 test containers in parallel
...
docker run -d --name integration-test-activemq \
    --network "$NETWORK_NAME" \
    -e SCHEDULER_FLAVOR=activemq \
    -e SCHEDULER_URL=http://scheduler-activemq:8080 \
    -e ACTIVEMQ_HOST=activemq-scheduler-test \
    -e ACTIVEMQ_PORT=61613 \
    scheduler-integration-tests
...
while true; do
    ...
    AMQ_STATUS=$(docker inspect integration-test-activemq --format='{{.State.Status}}' 2>/dev/null || echo "removed")

    if [ "$PG_STATUS" = "exited" ] && [ "$MYSQL_STATUS" = "exited" ] && [ "$CRDB_STATUS" = "exited" ] && [ "$AMQ_STATUS" = "exited" ]; then
        break
    fi

    echo "  Status: postgres=$PG_STATUS, mysql=$MYSQL_STATUS, cockroach=$CRDB_STATUS, activemq=$AMQ_STATUS"
    sleep 5
done
...
AMQ_EXIT=$(docker inspect integration-test-activemq --format='{{.State.ExitCode}}')

echo ""
echo "=== Test Results ==="

# Show results with logs for failures
for flavor in postgres mysql cockroach activemq; do
...
if [ "$PG_EXIT" != "0" ] || [ "$MYSQL_EXIT" != "0" ] || [ "$CRDB_EXIT" != "0" ] || [ "$AMQ_EXIT" != "0" ]; then
    exit 1
fi
